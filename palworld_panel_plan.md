...
配置分组：
- 配置编辑器由后端字段定义驱动，字段定义必须携带原始 Palworld key、前端 key、中文标签、分组、类型、说明、可选项和数字范围。
- 字段覆盖当前默认 `OptionSettings` 样本，而不是只覆盖早期常用字段；未知未来字段仍保留原样。
- 基础设置：服务器名称、描述、密码、管理员密码、最大玩家数、自动保存、官方备份。
- 网络与管理：Public IP/Port、REST API、RCON、跨平台、日志格式、玩家列表、加入离开提示、聊天频率、封禁列表。
- 性能：公会据点上限、据点工作帕鲁数、建筑上限、帕鲁同步距离、容器同步间隔。
- 倍率/战斗与生存：经验、捕获、刷新、掉落、采集、玩家/帕鲁伤害、饥饿、体力、回血、物品重量、装备耐久、复活惩罚。
- 世界与事件：随机化、昼夜速度、入侵、掉落物、孵蛋、补给/陨石、掠食者 Boss、工作速度。
- 据点与建筑、PvP、功能开关、属性强化、兼容/保留字段分别独立分组，避免配置页回到长页面。
