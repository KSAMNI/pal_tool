...
- Visual `PalWorldSettings.ini` editor driven by backend field definitions, covering the current default `OptionSettings` sample across basic server, network/management, performance, rates, survival/combat, world events, bases/building, PvP, feature switches, stat-enhancement, and reserved compatibility fields. Each field keeps the original Palworld key when rendering and exposes a concise in-UI description.
...
- Local MOD archive upload plus Steam Workshop download/install to `Mods/Workshop` or to the Steam client managed layout (`Mods/ManagedMods/<PackageName>` plus `Pal/Content/Paks/~WorkshopMods/<folder>`); upload/update requests reserve the backend operation slot before reading the archive body, request bodies are capped at 512 MiB, multipart parsing uses a smaller memory threshold so large archives spill to temporary files, Workshop downloads accept a numeric ID or Steam URL and use SteamCMD app `1623730`, extraction runs inside an operation-scoped workspace with file-count and byte limits, native ZIP uploads reject symlink entries, extracted MOD trees containing symlinks are rejected, downloaded Workshop content can install directly from `Info.json`, from one supported archive, or from a ManagedMods/Paks tree, external extractors are timeout/output bounded, staged MOD file copies stream through the atomic writer instead of reading whole files into memory, and final installed files are replaced only after SQLite commits the matching MOD row insert/update.
- MOD `Info.json` parsing with a metadata size cap, service-side support detection, PackageName validation before persistence/settings/path use, Workshop and `~WorkshopMods` folder-name validation before path reuse, enable/disable/update through `PalModSettings.ini`, selected-package update checks, and delete with backup plus `ManagedMods` and managed pak cleanup; MOD settings reads are capped at 1 MiB and oversized read APIs fail explicitly, MOD settings writes use atomic replacement, enable/disable commits the row before settings writes and restores the row if settings writes fail, install/update restores the previous database state if final replacement fails and reports previous-directory restore failures, and delete stages installed directories before cleanup so later failures can restore files plus compensate the database row and active settings.
- MOD upload/Workshop download/update/enable/disable/delete are blocked while PalServer is running; changes are made while stopped and take effect on the next start or restart.
- MOD rows can request the backend to open the installed MOD directory on the server machine with path constraints; managed pak installs open `Pal/Content/Paks/~WorkshopMods/<folder>`.
- Backend-proxied Palworld REST API dashboard for info, metrics, runtime settings, and online players; independent REST reads are fetched concurrently so dashboard refresh waits for the slowest call rather than accumulated endpoint latency, background dashboard refresh is throttled to reduce Palworld REST load, and upstream REST response bodies are capped with explicit oversized-response errors.
